...
IFILE="generated_trajectory_points-0.6.csv"
...
class FollowTrajectoryWrapper():
    ...
    def throwing_motion(self, trajectory):
        ...
        self._log("Time: {}".format(time()-self.time))

    def _callback(self, feedback):
        desired = feedback.desired.positions
        actual = feedback.actual.positions
        if not self._released and (np.allclose(desired[0:1], self._objective[0:1], rtol=10e-2) or np.allclose(actual[0:1], self._objective[0:1], rtol=10e-2)):
            self._log("Time from start: {}".format(time() - self.time))
            self._released = True
