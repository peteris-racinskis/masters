...
This allows for the employment of sequence-to-sequence predictor architectures also studied in other areas of machine learning such as recurrent neural networks and transformers. Finally, if continuous action spaces are permitted it is no great stretch to also consider cases where the action corresponds to a predicted future state that may be used for static motion planning or in a dynamic error-based controller

\begin{equation}
	\pi_{\theta}:\lbrace {(s_n)_1^t} \rbrace \rightarrow S
\end{equation}
which is equivalent to a sequence building task found in domains such as text generation. 
