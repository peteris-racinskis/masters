...
These adjustments allow for the employment of sequence-to-sequence predictor architectures also studied in other areas of machine learning such as recurrent neural networks (RNNs) \citep{rumelhart1985learning} and transformers \citep{vaswani2017attention}. Finally, if continuous action spaces are permitted it is no great stretch to also consider cases where the action corresponds to a predicted future state that may be used for static motion planning or in a dynamic error-based controller
...
\begin{itemize}
	...
	\item Model architecture -- what type of model template will be used to learn the policy?
	\item Control method -- how will model outputs be used to interface with the robot controller software and hardware?
\end{itemize}
