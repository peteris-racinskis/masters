...
\begin{figure}
	...
	\caption{Estimation of the throw target coordinates. Bottle position data is used to annotate each demonstration -- consisting of effector pose observations -- with throw target coordinates. Regression on the z-axis is used to find the ground plane intersection time, at which extrapolated x- and y-coordinate values are found. All quantities are plotted at the same scale, in units of meters.}
	\label{fig:fig3}
\end{figure}
...
In our research, numerous models were trained on both of the data sets described in section \ref{sec:collection}, in both of the architectures discussed in subsection \ref{sec:models}. After the initial trial and error process, a systematic training regimen produced the following:
