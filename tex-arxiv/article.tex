...
Hence, it was decided to compare model outputs with the demonstration data set. As judging against the training data set only informs us to the extent to which the model has been able to overfit, a smaller validation data set was set aside for out-of-distribution comparisons. The outputs to be compared were obtained by executing the models autoregresively on each demonstration's initial state, for as many steps as were present in the corresponding recorded demonstration.

Three broad classes of evaluation metrics were computed: data set-wise, step-wise and throw parameters. The first consist of vector similarity measures such as Pearson's correlation coefficient, cosine similarity and distance metrics applied to the entire data set and the trajectories generated against it. The second class involves applying various measures to the observation/state variables at each time step -- such as position or orientation deviations, or the categorical difference in gripper state. Finally, the release point of each trajectory was found, and the position and velocity vector differences were computed. When considering ways to combine these two error terms into a single quantitative estimator, it was decided that a simple ballistic extrapolation and resulting miss distance along the ground plane would serve as a decent first order approximation of throw accuracy \emph{vis-à-vis} the training or validation data set.
