...
As illustrated in fig. \ref{fig:vis_exec}, in terms of the qualitative aspects of executing a throw a successful result was attained with several models. Trajectories that visually resemble throwing motions were produced, and, when used to produce motion plans for a \emph{Universal Robotics Ur5e} robot equipped with a pneumatic gripper, they were adequate for throwing a plastic bottle into a target container located outside the robot's range of motion. Adequate output sequences were attained with both model architectures at multiple hyperparameter combinations, so to draw specific conclusions we need to look at the quantitative indicators of performance discussed above.
