...
Izvērtējot darba pamatmērķi -- tieši mašīnmācīšanas metožu pētīšanu --, pieejamos resursus un laika ierobežojumus, nolemts modeli konstruēt Dekarta koordināšu telpā kā autoregresoru diskrētā laikā. Iespējami MDP formālismam atbilstoši:
...
\begin{equation}
    t'_1 = min(t'_k) : t'_k > t_1
\end{equation}
\begin{equation}
    t'_p = min(t'_k) : t'_p > t_m
\end{equation}
...
\begin{equation}
    f_{freefall} (t) = \begin{cases}
        0 \text{ ja } \forall u < t,  \left [ \lVert \vec{r}_{Bottle}(u) - \vec{r}_{TrashPickup}(u)  \rVert \right ]'_u < \overline{v}_{freefall} \\
        1 \text{ citādi }
    \end{cases}
\end{equation}

Lai noteiktu, vai satvērējmehānisms ir sācis atlaisties, tas pats tiek darīts ar šī attāluma otrā atvasinājuma aproksimatoru:


\begin{equation}
    f_{release} (t) = \begin{cases}
        0 \text{ ja } \forall u < t,  \left [ \lVert \vec{r}_{Bottle}(u) - \vec{r}_{TrashPickup}(u)  \rVert \right ]''_u < \overline{a}_{release} \\
        1 \text{ citādi }
    \end{cases}
\end{equation}
...
\begin{equation}
    f_{moving} (t) = \begin{cases}
        0 \text{ ja } \forall u < t,  \lVert \left [ \vec{r}_{TrashPickup}(u) \right ]'_u \rVert  < \overline{v}_{moving} \\
        1 \text{ citādi }
    \end{cases}
\end{equation}
